...
interface AnnotationDetailsProps {
   ...
   onBoundingBoxUpdate: (boxId: string, updates: Partial<Annotation>) => void;
...
export const AnnotationDetails: React.FC<AnnotationDetailsProps> = ({
   ...
   const [boxes, setBoxes] = useState<Annotation[]>(annotations);
   ...
   const applyBulkLabel = (newLabel: string) => {
      ...
      ids.forEach((id) => {
         ...
         if (box) onBoundingBoxUpdate(id, { ...box, label: trimmed });
      });
   ...
   const commitEdit = (b: Annotation) => {
      onBoundingBoxUpdate(b.id, { ...b, label: labelValue.trim() });
   ...
   const applyFlag = (flagValue: string | null) => {
      if (flagTargetId) {
         ...
         if (box) {
            ...
            onBoundingBoxUpdate(flagTargetId, updated);
         }
      }
   ...
   const humanCoords = (b: Annotation) =>
      ...
      >
         ...
         <Box sx={{ px: 1 }}>
            ...
            <Slider
               ...
               size="small"
            />
            <Tooltip title={`Delete all annotations with score < ${confidence.toFixed(2)}`}>
               <span>
                  <Button
                     size="small"
                     variant="outlined"
                  ...
                  >
                     Remove annotations below score : {confidence.toFixed(2)}
                  </Button>
               </span>
            </Tooltip>
               ...
               <span>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, width: '100%', gap: 2 }}>
                      ...
                      <Button
                        ...
                        color="warning"
                        onClick={() => {
                           const boxesToRemove = applyNMS(boxes, nms);
         ...
         <Box sx={{ flex: 1, minHeight: 180, display: "flex", flexDirection: "column" }}>
            ...
            {/* Bulk action bar – visible when ≥2 items are multi-selected */}
         ...
         <Dialog open={bulkEditOpen} onClose={() => setBulkEditOpen(false)} maxWidth="xs" fullWidth>
            <DialogTitle>
               Edit label for {selectedIds.length} selected annotation{selectedIds.length !== 1 ? "s" : ""}
            </DialogTitle>
            <DialogContent>
               <TextField
                  variant="filled"
