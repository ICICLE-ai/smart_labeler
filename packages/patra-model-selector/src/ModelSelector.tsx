...
import { checkHfSecretExists, saveHfTokenToVault } from "./hfTokenVault";
import { getPatraModelDetails, listPatraModels } from "./patraClient";
import { PatraCard, PatraModelDetails } from "./types";
import { PatraDetailsContent } from "./PatraDetailsContent";
...
interface ModelSelectorProps {
    ...
    tapisToken?: string;
    /** Tapis username of the logged-in user — required to read/write their HF token in the Tapis vault. */
...
export const ModelSelector: React.FC<ModelSelectorProps> = ({
    ...
    useEffect(() => {
        ...
        getPatraModelDetails(pendingModelId, tapisToken)
    ...
    const openDetails = (uuid: string) => {
        ...
        getPatraModelDetails(uuid, tapisToken)
            .then((details: PatraModelDetails) => {
    ...
    const handleModelClick = async (uuid: string) => {
        ...
        if (card?.is_gated && tapisToken && tapisUsername) {
            const secretExists = await checkHfSecretExists(tapisToken, tapisUsername);
            if (!secretExists) {
    ...
    const saveHfToken = async () => {
        if (!tapisToken || !tapisUsername) {
            setHfTokenError("Tapis token or username not available");
            return;
        }

        setHfTokenSaving(true);
        const result = await saveHfTokenToVault(hfTokenValue, tapisToken, tapisUsername);
